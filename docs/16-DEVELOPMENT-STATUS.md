...
| FL-02 | توکن API برای برنامه‌نویسان | ساخت/لغو توکن شخصی workspace برای فراخوانی API و webhook | انجام شد | 2026-05-16 | `cbx_` Bearer؛ جدول api_tokens؛ تب API در تنظیمات |
...
| کد | عنوان | توضیح | وضعیت | تاریخ | یادداشت |
|----|-------|-------|--------|-------|---------|
| FL-05 | ابزارک (ویجت + پلاگین) | ویجت embed + پلاگین CMS (وردپرس و …) با برند «ابزارک» | انجام شد | 2026-05-17 | embed؛ تب «ابزارک» در تنظیمات؛ پلاگین وردپرس `chatbox-abzar`؛ `pnpm zip:wordpress`؛ `show_branding` برای پلن رایگان |
...
| FL-07 | گزارش گفتگوها | گزارش بر اساس بازه تاریخ، export، جستجو در متن/فیلترها | انجام شد | 2026-05-16 | GET /v1/reports/conversations؛ CSV export؛ صفحه /reports |
...
| کد | عنوان | توضیح | وضعیت | تاریخ | یادداشت |
|----|-------|-------|--------|-------|---------|
| FL-10 | اطلاعات بازدیدکننده | IP، URL صفحه فعلی، کشور (GeoIP)، دستگاه و مرورگر/OS در پنل اپراتور | انجام شد | 2026-05-17 | metadata.visitor؛ ویجت page_url؛ پنل inbox؛ WS visitor:context |
| FL-11 | سابقه بازدید صفحات | تاریخچه صفحاتی که مشتری در سایت بازدید کرده | انجام شد | 2026-05-17 | metadata.pageViews؛ لیست در پنل بازدیدکننده |

### دسته — تیم

| کد | عنوان | توضیح | وضعیت | تاریخ | یادداشت |
|----|-------|-------|--------|-------|---------|
| FL-12 | آواتار اپراتور | آپلود/تنظیم آواتار برای هر اپراتور (چت + لیست تیم) | انجام شد | 2026-05-17 | `POST/DELETE /v1/auth/me/avatar`؛ تنظیمات پروفایل؛ نمایش در MessageThread و TeamPanel |
